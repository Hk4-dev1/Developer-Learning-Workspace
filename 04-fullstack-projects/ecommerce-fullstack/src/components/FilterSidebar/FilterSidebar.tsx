...
interface FilterSidebarProps {
  ...
  categories?: string[]; // Made optional since we'll get from context
  brands?: string[]; // Made optional since we'll get from context
  priceRange?: PriceRange; // Made optional with default
}
...
const FilterSidebar: React.FC<FilterSidebarProps> = ({
  ...
  brands: propsBrands = [],
  priceRange: propsPriceRange = { min: 0, max: 5000 }
}) => {
  const { categories: apiCategories, brands: apiBrands } = useProductContext();
  ...
  const categories = apiCategories?.map(cat => cat.name) || propsCategories;
  const brands = apiBrands?.map(brand => brand.name) || propsBrands;
  const priceRange = propsPriceRange;
